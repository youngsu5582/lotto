...
def generate_lotto_insert_statements(output_file: str, batch_size: int = 1000):
    ...
    combinations_count = 0  # 생성된 조합의 개수 카운트

    with open(output_file, 'w') as file:
        # 테이블 생성 스크립트 작성
        file.write("CREATE TABLE IF NOT EXISTS lotto (\n")
        file.write("  id BIGINT NOT NULL AUTO_INCREMENT,\n")
        ...
        for combo in combinations(numbers, 6):
            ...
            values.append(f"('{formatted_combination}')")
            combinations_count += 1

            # 배치 크기에 도달하면 INSERT 문 작성
            if len(values) == batch_size:
                # 미리 문자열 생성 후 작성
                insert_statement = "INSERT INTO lotto (numbers) VALUES\n" + ",\n".join(values) + ";\n\n"
    ...
    print(f"{combinations_count}개의 조합이 생성되어 {output_file}에 저장되었습니다.")
